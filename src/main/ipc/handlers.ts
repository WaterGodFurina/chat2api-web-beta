...
let ipcMain: any, app: any, BrowserWindow: any, shell: any
try {
...
export async function registerIpcHandlers(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  if (config.autoStartProxy) {
    console.log('[App] Auto-start proxy is enabled, but we defer it to manual control to avoid port conflicts during boot')
  }
  ...
  ipcMain.handle(IpcChannels.PROXY_START, async (_, port?: number): Promise<boolean> => {
    try {
      ...
      const config = storeManager.getConfig()
      const proxyPort = port || config.proxyPort || 8080
      const proxyHost = config.proxyHost || '0.0.0.0'

      console.log(`[IPC] Starting proxy on ${proxyHost}:${proxyPort} (requested port: ${port}, config port: ${config.proxyPort})`)
  ...
  ipcMain.handle(IpcChannels.PROXY_GET_STATUS, async (): Promise<ProxyStatus> => {
    ...
    const host = proxyStatusManager.getConfig().host
    return {
      isRunning,
      port,
      host,
