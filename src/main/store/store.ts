 ...
 * Uses FileStore for JSON file storage (Web mode)
 */
...
// Electron-safe imports with fallbacks to shims
let app: any = { getPath: (name: string) => join(homedir(), '.chat2api'), getVersion: () => '1.3.0' }
let safeStorage: any = { isEncryptionAvailable: () => false, encryptString: (s: string) => Buffer.from(s, 'utf-8'), decryptString: (b: Buffer) => b.toString('utf-8') }
...
try {
  ...
  const shims = _require('../../server/electron-shims')
  app = shims.app
  safeStorage = shims.safeStorage
  BrowserWindow = shims.BrowserWindow
}

// Dynamically import electron-store (ESM module), fallback to FileStore
let Store: any = null
...
class StoreManager {
  ...
  async initialize(): Promise<void> {
    ...
    // Try electron-store first, fallback to FileStore for web mode
    if (!Store) {
      try {
        const module = await import('electron-store')
        Store = module.default
      } catch {
        // electron-store not available, use FileStore
        console.log('[Store] electron-store not available, using FileStore (web mode)')
        const { default: FileStore } = await import('../../server/fileStore')
        Store = FileStore
      }
    }
    ...
    try {
      ...
      // Only add encryptionKey for electron-store
      if (Store.name !== 'FileStore') {
        storeOptions.encryptionKey = this.getEncryptionKey()
      }
      ...
      try {
        ...
        if (Store.name !== 'FileStore') {
          recoveryOptions.encryptionKey = this.getEncryptionKey()
        }
        this.store = new Store(recoveryOptions)
        await this.initializeAppLogManager(storagePath)
        await this.initializeRequestLogManager(storagePath)
        this.isInitialized = true
        this.initializationError = null
  ...
  /**
   * Get Storage Path
   * Storage path: Specified by CHAT2API_DATA_DIR or /app/data (Docker) or ~/.chat2api/ (Local)
   */
  private getStoragePath(): string {
    // 1. Environment Variable Priority (Standard for Docker/Server)
   ...
   * Get Request Logs
   */
  getRequestLogs(limit?: number, filter?: { status?: 'success' | 'error'; providerId?: string; apiKeyId?: string }): RequestLogEntry[] {
    this.ensureInitialized()
