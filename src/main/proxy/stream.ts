...
/**
 * Clean content by removing CHAT2API tags, User:/Assistant: prefixes, and other unwanted patterns
 */
function cleanContentOutput(content: string): string {
  if (!content) return content

  let cleaned = content

  // Remove CHAT2API tool_result tags and their content
  cleaned = cleaned.replace(/<\|CHAT2API\|tool_result[^>]*\|*>[\s\S]*?(?:<\/\|CHAT2API\|tool_result\|*>|$)/g, '')

  // Remove CHAT2API tool_calls tags and their content
  cleaned = cleaned.replace(/<\|CHAT2API\|tool_calls\|*>[\s\S]*?(?:<\/\|CHAT2API\|tool_calls\|*>|$)/g, '')

  // Remove any remaining CHAT2API tags (opening and closing)
  cleaned = cleaned.replace(/<\|CHAT2API\|[^>]*\|*>/g, '')
  cleaned = cleaned.replace(/<\/\|CHAT2API\|[^>]*\|*>/g, '')

  // Remove "User: <CHAT2API|tool_result...>" patterns
  cleaned = cleaned.replace(/User:\s*<\|CHAT2API\|tool_result[^>]*\|*>[\s\S]*?(?:<\/\|CHAT2API\|tool_result\|*>|$)/g, '')

  // Remove "Assistant: " prefix at the start of content or after newlines
  cleaned = cleaned.replace(/^(Assistant:\s*)+/g, '')
  cleaned = cleaned.replace(/\n(Assistant:\s*)+/g, '\n')

  // Remove "User: " prefix at the start of content or after newlines (when followed by CHAT2API or tool-related content)
  cleaned = cleaned.replace(/^User:\s*/g, '')
  cleaned = cleaned.replace(/\nUser:\s*/g, '\n')

  // Remove [function_calls] blocks and their content (AstrBot/managed bracket format)
  cleaned = cleaned.replace(/\[function_calls\][\s\S]*?\[\/function_calls\]/g, '')

  // Remove AstrBot-style tool call text patterns
  cleaned = cleaned.replace(/(?:调用工具|正在调用|调用函数|Calling tool|Invoking tool|Using tool)[：:\s]*\S+/gi, '')
        ...
        try {
          ...
          for (const event of events) {
            if (event.data === '[DONE]') {
              ...
              if (contentBuffer) {
                const cleanedBuffer = cleanContentOutput(contentBuffer)
                if (cleanedBuffer) {
                  const finalData = transformChunk({ content: cleanedBuffer }, model, responseId, created, isFirstChunk)
                  if (finalData) {
                    isFirstChunk = false
            ...
            if (deltaContent) {
              ...
              if (!isBufferingToolCall) {
                ...
                for (const marker of markers) {
                  ...
                  if (markerIdx !== -1) {
                    isBufferingToolCall = true
                    foundMarker = true
                    // If we have text before the marker, send it first (after cleaning)
                    if (markerIdx > 0) {
                      const textBefore = cleanContentOutput(contentBuffer.substring(0, markerIdx))
                      if (textBefore) {
                        const textData = {
                          ...transformedData,
                          choices: [{
                    ...
                    for (let i = 0; i < contentBuffer.length; i++) {
                      if (contentBuffer[i] === marker[0]) {
                        const potentialMarker = contentBuffer.substring(i)
                        if (marker.startsWith(potentialMarker)) {
                          isBufferingToolCall = true
                          foundMarker = true
                          if (i > 0) {
                            const textBefore = cleanContentOutput(contentBuffer.substring(0, i))
                            if (textBefore) {
                              const textData = {
                                ...transformedData,
                                choices: [{
              ...
              if (isBufferingToolCall) {
                ...
                if (toolCalls.length > 0) {
                  ...
                  isBufferingToolCall = contentBuffer.includes('[function_calls]') || contentBuffer.includes('<|CHAT2API|tool_calls>')
              ...
              // Normal text output - apply content cleaning
              const cleanedContent = cleanContentOutput(contentBuffer)
              if (cleanedContent) {
                transformedData.choices[0].delta!.content = cleanedContent
              } else {
      ...
      flush(callback) {
        if (contentBuffer) {
          ...
          if (toolCalls.length > 0) {
            ...
            // Flush any remaining clean content after tool calls
            if (cleanContent) {
              const cleanedRemainder = cleanContentOutput(cleanContent)
              if (cleanedRemainder) {
                const finalData = transformChunk({ content: cleanedRemainder }, model, responseId, created, isFirstChunk)
                if (finalData) {
                  this.push(formatter.formatJSON(finalData))
                }
              }
            ...
            // No tool calls, just flush content (after cleaning)
            const cleanedBuffer = cleanContentOutput(contentBuffer)
            if (cleanedBuffer) {
              const finalData = transformChunk({ content: cleanedBuffer }, model, responseId, created, isFirstChunk)
              if (finalData) {
  ...
  async streamToResponse(
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.on('end', () => {
        ...
        // Also apply content cleaning to remove CHAT2API tags and User:/Assistant: prefixes
        const finalContent = finalToolCalls.length > 0 ? null : (cleanContentOutput(cleanContent) || null)
