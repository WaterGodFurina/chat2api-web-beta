...
export class RequestForwarder {
  ...
  private async doForward(
    ...
    try {
      ...
      // Apply HTTP proxy if configured for this account
      if (account.httpProxy) {
        try {
          axiosConfig.httpsAgent = new HttpsProxyAgent(account.httpProxy)
          axiosConfig.httpAgent = new HttpsProxyAgent(account.httpProxy)
        } catch (proxyErr) {
          console.warn(`[Forwarder] Invalid proxy URL for account ${account.id}: ${account.httpProxy}`, proxyErr)
        }
      }
