...
export class GLMAdapter {
  ...
  /**
   * Create axios config with HTTP proxy support from account settings
   */
  private createAxiosConfig(overrides: AxiosRequestConfig = {}): AxiosRequestConfig {
    return createAxiosConfig(this.account.httpProxy, overrides)
  }
  ...
  private async uploadFile(fileUrl: string): Promise<{ source_id: string; file_url?: string }> {
    ...
    if (this.isBase64Data(fileUrl)) {
      ...
      const response = await axios.get(fileUrl, this.createAxiosConfig({
  ...
  private messagesToPrompt(messages: GLMMessage[], refs: any[] = [], toolsPrompt?: string, isMultiTurn: boolean = false): { role: string; content: any[] }[] {
    ...
    // Add image references (GLM native format — uses 'type:image' with source_id, not OpenAI image_url)
    if (imageRefs.length > 0) {
      content.push({
        type: 'image',
        image: imageRefs.map(ref => ({
          source_id: ref.source_id,
          file_url: ref.image_url || ref.source_id,
          width: ref.width || 0,
          height: ref.height || 0,
        })),
      })
    ...
    // When images or files are present, inject attention prompt (referencing GLM-Free-API approach)
    const hasFilesOrImages = imageRefs.length > 0 || fileRefs.length > 0
    if (hasFilesOrImages) {
      // Inject a system message to boost attention to files/images
      messages = [...messages]
      ...
      if (lastUserIdx !== -1) {
        messages.splice(lastUserIdx, 0, attentionMessage)
      } else {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse; conversationId: string }> {
    ...
    if (needsToolPromptInjection) {
      // Legacy fallback: inject toolsToSystemPrompt directly
      // This path is only reached when ToolCallingEngine is NOT active
      ...
      for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].role === 'user') {
          ...
          if (typeof currentContent === 'string') {
            messages[i] = { ...messages[i], content: currentContent + wrapHint }
