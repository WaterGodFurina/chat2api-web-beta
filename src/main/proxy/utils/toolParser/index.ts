...
export function parseToolCalls(content: string): ToolParseResult {
  ...
  switch (format) {
    ...
    case 'json':
      ...
      break
    case 'chat2api_xml':
      result = parseChat2ApiXmlFormat(content)
      break
    default:
      return { content, toolCalls: [], format: 'unknown', rawMatches: [] }
  }
 ...
 */
/**
 * 解析 CHAT2API XML 格式: <|CHAT2API|tool_calls><|CHAT2API|invoke name="tool_name"><|CHAT2API|parameter name="arg"><![CDATA[value]]></|CHAT2API|parameter></|CHAT2API|invoke></|CHAT2API|tool_calls>
 */
function parseChat2ApiXmlFormat(content: string): { content: string; toolCalls: ToolCall[]; rawMatches: string[] } {
  const toolCalls: ToolCall[] = []
  const rawMatches: string[] = []
  let cleanContent = content

  // Match the entire CHAT2API tool_calls block
  const blockRegex = /<\|CHAT2API\|tool_calls\|*>([\s\S]*?)(?:<\/\|CHAT2API\|tool_calls\|*>|$)/g
  let blockMatch

  while ((blockMatch = blockRegex.exec(content)) !== null) {
    const blockContent = blockMatch[0]
    rawMatches.push(blockContent)

    // Match invoke elements within the block
    const invokeRegex = /<\|CHAT2API\|invoke\s+name="([^"]+)"\|*>([\s\S]*?)(?:<\/\|CHAT2API\|invoke\|*>|$)/g
    let invokeMatch
    ...
    while ((invokeMatch = invokeRegex.exec(blockContent)) !== null) {
      ...
      const paramRegex = /<\|CHAT2API\|parameter\s+name="([^"]+)"\|*>([\s\S]*?)(?:<\/\|CHAT2API\|parameter\|*>|$)/g
      let paramMatch
  ...
  // Also match and remove CHAT2API tool_result blocks
  const toolResultRegex = /<\|CHAT2API\|tool_result[^>]*\|*>[\s\S]*?(?:<\/\|CHAT2API\|tool_result\|*>|$)/g
  let toolResultMatch
  while ((toolResultMatch = toolResultRegex.exec(content)) !== null) {
    rawMatches.push(toolResultMatch[0])
  }

  // Remove all matched content from clean content
  for (const raw of rawMatches) {
    cleanContent = cleanContent.replace(raw, '')
  }

  // Also clean up any remaining CHAT2API tags that weren't matched
  cleanContent = cleanContent.replace(/<\|CHAT2API\|[^>]*\|*>/g, '')
  cleanContent = cleanContent.replace(/<\/\|CHAT2API\|[^>]*\|*>/g, '')
