...
function renderPrompt(
  ...
  let prompt = getToolProtocol(protocol).renderPrompt(tools)

  if (config.clientAdapterId === 'astrbot') {
    prompt += '\n\nIMPORTANT: If you decide to call a tool, you MUST wrap the tool name in exactly this tag: <|chat2api|tool_name|chat2api|>.\nExample: If you want to use the web_search tool, you MUST use the string <|chat2api|web_search|chat2api|> as the tool name.'
  }
