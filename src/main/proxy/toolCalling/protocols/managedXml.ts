import type { ToolProtocolAdapter } from './base.ts'
import type { ToolParseContext } from '../types.ts'
import {
  addParameter,
  buildToolCall,
  createParseResult,
  detectMarkers,
  escapeXmlAttribute,
  parseJsonValue,
  renderToolList,
  stripFencedCodeBlocks,
  toolNames,
} from './shared.ts'

const CHAT2API_START = '<|CHAT2API|tool_calls>'
const CHAT2API_END = '</|CHAT2API|tool_calls>'
const XML_START = '<tool_calls>'

// Helper to avoid esbuild parsing | inside template literals as TypeScript union types
const $ = '|CHAT2API|'
const tag = (name: string) => `<${$}${name}>`
const tagEnd = (name: string) => `</${$}${name}>`

export const managedXmlProtocol: ToolProtocolAdapter = {
  id: 'managed_xml',

  renderPrompt(tools) {
    const T = (name: string) => tag(name)
    const TE = (name: string) => tagEnd(name)
    return `## Available Tools
You have access to the following developer tools. Tool names are case-sensitive.
You MUST use only the exact tool names listed below. Do not rename, camelCase, translate, shorten, or invent tool names.

${renderToolList(tools)}

## CRITICAL RULES FOR TOOL USAGE
1. When a user request can be fulfilled by one or more of the available tools, you MUST call the appropriate tool(s) instead of responding with plain text.
2. NEVER refuse or apologize when a tool is available to fulfill the request. Do NOT say "I cannot", "I'm unable to", "I don't have access to", or "I'm sorry, I can't help with that" when a relevant tool exists.
3. If you are unsure whether a tool can help, TRY the tool first rather than refusing.
4. Always call tools with the EXACT tool names and parameter names as defined above.
5. When multiple tools are needed, call them all in a single response.

When calling tools, respond with ONLY this Chat2API XML block:

${T('tool_calls')}${T('invoke name="exact_tool_name"')}${T('parameter name="param1"')}<![CDATA[value1]]>${TE('parameter')}${T('parameter name="param2"')}<![CDATA[value2]]>${TE('parameter')}${TE('invoke')}${TE('tool_calls')}

CRITICAL XML RULES:
- Each parameter MUST have its own complete ${T('parameter name="..."')}...${TE('parameter')} pair.
- Do NOT combine or merge parameters. The closing ${TE('parameter')} MUST NOT contain any attribute names.
- Example of CORRECT multiple parameters:
  ${T('parameter name="query"')}<![CDATA[search text]]>${TE('parameter')}${T('parameter name="count"')}<![CDATA[10]]>${TE('parameter')}
- Example of WRONG format (do NOT do this):
  ${T('parameter name="query"')}<![CDATA[text]]>${TE('parameter name="count"')}<![CDATA[10]]>${TE('parameter')}

Tool results will be provided as Chat2API XML result blocks:

${T('tool_result tool_call_id="call_id"')}<![CDATA[result]]>${TE('tool_result')}

After receiving tool results, you MUST synthesize the results and provide a helpful response to the user. Do NOT simply repeat the raw tool output.`
  },

  detectStart(buffer) {
    return detectMarkers(buffer, [CHAT2API_START, XML_START])
  },

  parse(content: string, context: ToolParseContext) {
    const parseable = stripFencedCodeBlocks(content)
    const allowedNames = toolNames(context.tools)
    const rawMatches: string[] = []
    const invalidToolNames: string[] = []
    const toolCalls = []

    parseBlocks(parseable, {
      blockPattern: /<\|CHAT2API\|tool_calls>([\s\S]*?)<\/\|CHAT2API\|tool_calls>/g,
      invokePattern: /<\|CHAT2API\|invoke\s+name="([^"]+)"\s*>([\s\S]*?)<\/\|CHAT2API\|invoke>/g,
      parameterPattern: /<\|CHAT2API\|parameter\s+name="([^"]+)"\s*>([\s\S]*?)<\/\|CHAT2API\|parameter>/g,
      rawMatches,
      invalidToolNames,
      allowedNames,
      toolCalls,
    })

    parseBlocks(parseable, {
      blockPattern: /<tool_calls>([\s\S]*?)<\/tool_calls>/g,
      invokePattern: /<invoke\s+name="([^"]+)"\s*>([\s\S]*?)<\/invoke>/g,
      parameterPattern: /<parameter\s+name="([^"]+)"\s*>([\s\S]*?)<\/parameter>/g,
      rawMatches,
      invalidToolNames,
      allowedNames,
      toolCalls,
    })

    if (toolCalls.length === 0) {
      return createParseResult({
        content,
        toolCalls,
        protocol: rawMatches.length > 0 ? 'managed_xml' : 'unknown',
        rawMatches,
        invalidToolNames,
      })
    }

    const cleanContent = rawMatches.reduce((acc, raw) => acc.replace(raw, ''), parseable).trim()
    return createParseResult({
      content: cleanContent,
      toolCalls,
      protocol: 'managed_xml',
      rawMatches,
      invalidToolNames,
    })
  },

  formatAssistantToolCalls(calls) {
    const invokes = calls.map((call) => {
      const args = safeParseObject(call.arguments)
      const params = Object.entries(args)
        .map(([name, value]) => {
          const text = typeof value === 'string' ? value : JSON.stringify(value)
          return `${tag('parameter name="' + escapeXmlAttribute(name) + '"')}<![CDATA[${text}]]>${tagEnd('parameter')}`
        })
        .join('')
      return `${tag('invoke name="' + escapeXmlAttribute(call.name) + '"')}${params}${tagEnd('invoke')}`
    })
    return `${CHAT2API_START}${invokes.join('')}${CHAT2API_END}`
  },

  formatToolResult(result) {
    return `${tag('tool_result tool_call_id="' + escapeXmlAttribute(result.toolCallId) + '"')}<![CDATA[${result.content}]]>${tagEnd('tool_result')}`
  },
}

interface ParseBlockOptions {
  blockPattern: RegExp
  invokePattern: RegExp
  parameterPattern: RegExp
  rawMatches: string[]
  invalidToolNames: string[]
  allowedNames: Set<string>
  toolCalls: ReturnType<typeof buildToolCall>[]
}

/**
 * Normalize malformed XML parameter tags.
 * 
 * Some models (e.g. GLM) generate malformed XML where the closing tag
 * is immediately followed by the next parameter's name attribute,
 * e.g. `</|CHAT2API|parameter name="search_depth">` instead of
 * `</|CHAT2API|parameter><|CHAT2API|parameter name="search_depth">`.
 * 
 * This function converts the malformed form to the correct one.
 */
function normalizeMalformedXml(xml: string, tagName: string): string {
  // Match: </tagName name="..."> → replace with: </tagName><tagName name="...">
  const pattern = new RegExp(`</${tagName}\\s+name="([^"]*)"\\s*>`, 'g')
  return xml.replace(pattern, (match, name) => `</${tagName}><${tagName} name="${name}">`)
}

function parseBlocks(content: string, options: ParseBlockOptions): void {
  let blockMatch: RegExpExecArray | null

  while ((blockMatch = options.blockPattern.exec(content)) !== null) {
    options.rawMatches.push(blockMatch[0])
    let invokeMatch: RegExpExecArray | null

    // Normalize malformed XML inside the block before parsing invokes
    let blockContent = normalizeMalformedXml(blockMatch[1], '|CHAT2API|parameter')
    blockContent = normalizeMalformedXml(blockContent, '|CHAT2API|invoke')
    // Also normalize the short-form (without CHAT2API prefix)
    blockContent = normalizeMalformedXml(blockContent, 'parameter')
    blockContent = normalizeMalformedXml(blockContent, 'invoke')

    while ((invokeMatch = options.invokePattern.exec(blockContent)) !== null) {
      let name = invokeMatch[1].trim()
      name = name.replace(/<\|chat2api\|/g, '').replace(/\|chat2api\|>/g, '')
      if (!options.allowedNames.has(name)) {
        options.invalidToolNames.push(name)
        continue
      }

      const args: Record<string, unknown> = {}
      let parameterMatch: RegExpExecArray | null
      options.parameterPattern.lastIndex = 0
      while ((parameterMatch = options.parameterPattern.exec(invokeMatch[2])) !== null) {
        addParameter(args, parameterMatch[1].trim(), parseJsonValue(parameterMatch[2]))
      }

      options.toolCalls.push(
        buildToolCall(`call_${options.toolCalls.length}`, options.toolCalls.length, name, JSON.stringify(args), invokeMatch[0]),
      )
    }
  }
}

function safeParseObject(value: string): Record<string, unknown> {
  try {
    const parsed = JSON.parse(value)
    return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {}
  } catch {
    return {}
  }
}
