...
export function ProxyConfigForm({ onConfigChange }: ProxyConfigFormProps) {
  ...
  const { proxyConfig, proxyStatus, appConfig, setProxyConfig, saveAppConfig, startProxy, stopProxy, isLoading } = useProxyStore()
  ...
  const handleStreamingToggle = (enabled: boolean) => {
    setFormData(prev => ({ ...prev, enableStreaming: enabled }))
    setHasChanges(true)
    onConfigChange?.()
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-2 border-t">
            <div className="space-y-0.5">
              <Label htmlFor="streaming-toggle">{t('proxy.enableStreaming')}</Label>
              <p className="text-sm text-muted-foreground">
                {t('proxy.enableStreamingHelp')}
              </p>
            </div>
            <Switch
              id="streaming-toggle"
