...
import { Copy, Check, Clock, Zap, Server, User, FileJson, AlertCircle, Globe, Brain, MessageSquare } from 'lucide-react'
      ...
      <DialogContent className="max-w-3xl w-[90vw] h-[85vh] max-h-[800px] p-0 flex flex-col overflow-hidden bg-background/95 backdrop-blur-sm">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 px-6 py-4 overflow-y-auto">
            ...
            {/* System Prompt Tab */}
            <TabsContent value="systemPrompt" className="mt-0">
              {log.systemPrompt ? (
                <div>
                  <SectionHeader 
                    title={t('logs.systemPrompt')} 
                    icon={<MessageSquare className="h-4 w-4" />}
                    copyText={log.systemPrompt}
                  />
                  <div className="p-4 bg-muted/30 rounded-lg border border-border/50">
                    <pre className="text-sm whitespace-pre-wrap break-all font-sans leading-relaxed">
                      {log.systemPrompt}
                ...
                <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">{t('logs.noSystemPrompt')}</p>
                </div>
              )}
            </TabsContent>
          </div>
