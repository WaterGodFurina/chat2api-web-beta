...
export function RequestLogList() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      if (apiKeyFilter !== 'all') filter.apiKeyId = apiKeyFilter
  ...
  // Fetch API keys from config for the filter dropdown
  useEffect(() => {
    const fetchApiKeys = async () => {
      try {
        const config = await window.electronAPI?.config?.get()
        if (config?.apiKeys) {
          setApiKeys(config.apiKeys.map((k: any) => ({ id: k.id, name: k.name })))
        }
      } catch (error) {
    ...
    fetchApiKeys()
    // Refresh API keys every 10 seconds
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                ...
                {log.apiKeyName && (
                  <>
                    <span>·</span>
                    <span className="text-blue-500 dark:text-blue-400">{log.apiKeyName}</span>
                  </>
                )}
              </div>
            </div>
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-4">
          ...
          <Select value={apiKeyFilter} onValueChange={(v) => setApiKeyFilter(v)}>
            <SelectTrigger className="w-40">
              <SelectValue placeholder={t('logs.allApiKeys', 'All API Keys')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('logs.allApiKeys', 'All API Keys')}</SelectItem>
              {apiKeys.map((key) => (
